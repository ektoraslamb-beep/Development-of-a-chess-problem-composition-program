...
from auw_fitness import evaluate_auw_fitness

# ================= ΡΥΘΜΙΣΕΙΣ GA ===================

SEEDS_CSV = r"C:\Users\Ektoras\sxoli\Diplwmatiki\mate_2_data\mate_2_data_full.csv"
POPULATION_SIZE = 250
GENERATIONS = 1000
ELITE_COUNT = 10
MUTATIONS_PER_CHILD = (5, 15)
CANDIDATES_CSV = "auw_candidates.csv"
CANDIDATE_FITNESS_THRESHOLD = 800.0
CROSSOVER_RATE = 0.6
...
def crossover_boards(board1: chess.Board, board2: chess.Board, max_tries=10) -> chess.Board:
    """
    Random square mask crossover:
    - Κάθε τετράγωνο κληρώνεται τυχαία σε γονέα 1 ή γονέα 2
    - Αν mask[sq] == True  → παίρνουμε το κομμάτι από τον board1
    - Αν mask[sq] == False → παίρνουμε το κομμάτι από τον board2
    - Βασιλιάδες: πάντα από τον αντίστοιχο γονέα (board1→λευκός, board2→μαύρος)
    - Fallback στον board1 αν δεν βγει valid θέση μετά από max_tries
    """
    for _ in range(max_tries):
        new_board = chess.Board(fen=None)
        new_board.clear()

        # Βασιλιάδες πάντα από τον αντίστοιχο γονέα
        ...
        # Τυχαία μάσκα: κάθε τετράγωνο → γονέας 1 ή 2
        mask = [random.random() < 0.5 for _ in range(64)]

        # Κομμάτια από γονέα 1 (εκτός βασιλιά)
        for sq, piece in board1.piece_map().items():
            if piece.piece_type == chess.KING:
                continue
            if mask[sq] and new_board.piece_at(sq) is None:
                new_board.set_piece_at(sq, piece)

        # Κομμάτια από γονέα 2 (εκτός βασιλιά)
        for sq, piece in board2.piece_map().items():
            if piece.piece_type == chess.KING:
                continue
            if not mask[sq] and new_board.piece_at(sq) is None:
                new_board.set_piece_at(sq, piece)
...
class Individual:
    ...
    def evaluate(self, engine):
        score, hero_move, details = evaluate_auw_fitness(self.board, engine)
        self.fitness = score
...
def init_population(seed_fens, pop_size: int):
    ...
    best_seed_fen = "n6k/1r6/1Q1R1P2/8/n3Kn2/4n3/BB6/8 w - - 0 1"

    # 20% του πληθυσμού από το καλό seed με ελαφρύ mutation (random immigration seed)
    num_best_seeds = int(pop_size * 0.20)
    for _ in range(num_best_seeds):
        b = chess.Board(best_seed_fen)
        mutate_board(b, random.randint(1, 2))
        population.append(Individual(b))

    # Υπόλοιπος πληθυσμός από τα seeds του CSV
...
def make_next_generation(population, seed_fens):
    ...
    # Random immigration: 20 νέα άτομα από seeds (αποφυγή premature convergence)
    for _ in range(20):
        fen = random.choice(seed_fens)
        b = chess.Board(fen)
        mutate_board(b, random.randint(2, 5))
        new_pop.append(Individual(b))

    # Crossover + Mutation για τον υπόλοιπο πληθυσμό
...
def main():
    ...
    if not seed_fens:
        print("[!] Δεν βρέθηκαν seeds από το CSV.")
        return

    print(f"[i] Φορτώθηκαν {len(seed_fens)} seeds από το {SEEDS_CSV}.")
    ...
    for gen in range(GENERATIONS):
        ...
        population = make_next_generation(population, seed_fens)
    ...
    with open(CANDIDATES_CSV, "a", newline="", encoding="utf-8") as cand_file:
        ...
        if not file_exists:
            cand_writer.writerow(["fitness", "duration", "fen", "hero_uci", "sf_cp_score", "solutions"])
    ...
    print("\n" + "="*50)
    print(f"[i] GA AUW Phase ολοκληρώθηκε.")
